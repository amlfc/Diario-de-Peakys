...
export interface ClosedTrade {
  ...
  // Moneda de la operacion (USD, GBP, CHF, etc.)
  currency: string;
...
export const calculateClosedTrades = (transactions: Transaction[]): ClosedTrade[] => {
  ...
  type Lot = {
    remainingQty: number;
    unitCostOrigin: number; // coste unitario en moneda (incluye comision prorrateada)
  ...
  sortedTxs.forEach((rawTx, index) => {
    ...
    // no deben contaminar el historico de operaciones cerradas.
    ...
    if (tx.type === TransactionType.Buy) {
      // BUY (moneda)
      const buyCostOrigin = (tx.price * qty) + tx.commission; // comision en moneda
    ...
    if (tx.type === TransactionType.Sell) {
      ...
      // En el histórico cerrado mostramos el equivalente en EUR de la operación
      // usando el FX de la venta, para que case con el P&L en moneda origen.
      const sellRevenueNetEur = sellRevenueNetOrigin * fx;
      const fifoCostEurAtSellFx = fifoCostOrigin * fx;
      const pnlEur = pnlOrigin * fx;

      const returnPct = fifoCostOrigin !== 0 ? (pnlOrigin / fifoCostOrigin) : 0;
      ...
      closedTrades.push({
        ...
        costBasisEur: avgCostOrigin * fx,
        grossRevenueEur: sellRevenueNetEur,
        grossCostEur: fifoCostEurAtSellFx,
        netPnLEur: pnlEur,
